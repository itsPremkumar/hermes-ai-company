...
> **Current live org:** lean 14-bot fleet (10 core + 4 agentic specialists). The `bots/` folder retains kits for the full historical 36-bot roster — deploy any subset.
---

## 1. The Company in One Paragraph

36 named Hermes bots (each a real Hermes profile) form an IT company: an executive team that
...
| [`docs/02-fleet-roster.md`](docs/02-fleet-roster.md) | All 34+ bots: role, team, model pin, toolset, assigned skills |
...
| [`docs/10-langgraph-projects.md`](docs/10-langgraph-projects.md) | 10 LangGraph agentic AI projects build program |
...
| [`docs/14-testing-standards.md`](docs/14-testing-standards.md) | QA baseline: coverage thresholds, test types, gate rules |
| [`docs/15-code-review-checklist.md`](docs/15-code-review-checklist.md) | Review criteria (correctness, performance, security, style) |
| [`docs/16-incident-response.md`](docs/16-incident-response.md) | Runbook for 8 incident types (gateway down, RAM, security…) |
...
| [`souls/`](souls/) | All 36 bot SOUL.md personas, organized by team (+ index README) |
...
├── docs\                  ← the 16 manuals above
├── souls\                 ← every bot's SOUL.md by team (executive/, delivery/, …)
├── bots\                  ← 📁 one folder per bot (36): SOUL.md + config.yaml + skills/ + README
...
| Proven output | 5 repos built→QA'd→pushed autonomously |
