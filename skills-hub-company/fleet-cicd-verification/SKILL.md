---
name: fleet-cicd-verification
description: Build, run, and maintain a universal cross-verifying CI/CD + closed-loop auto-improvement system across a FLEET of repositories (not one). Use when the user asks for "verify every project", "CI/CD for all repos", "self-improving deploy loop", "cross-check before marking done", "daily verification gate", or "find everything currently missing across my projects". Encodes the 8-angle cross-verification model, the daily closed-loop orchestrator pattern, and the Windows/MSYS path + os.walk pruning pitfalls that otherwise burn an entire session.
---
