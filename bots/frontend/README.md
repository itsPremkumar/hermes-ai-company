...
| Tools enabled | `file`, `image_gen`, `skills`, `todo` |
...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/frontend/SOUL.md`) |
...
```bash
# create
hermes profile create frontend --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/frontend/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/frontend/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/frontend/.env   # REQUIRED: keys
# use
