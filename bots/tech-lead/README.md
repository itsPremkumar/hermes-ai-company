...
| Tools enabled | `terminal`, `file`, `skills`, `todo`, `session_search` |
...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/tech-lead/SOUL.md`) |
...
```bash
# create
hermes profile create tech-lead --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/tech-lead/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/tech-lead/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/tech-lead/.env   # REQUIRED: keys
# use
hermes -p tech-lead chat
```
