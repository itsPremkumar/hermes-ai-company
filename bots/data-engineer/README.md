...
| Fallback chain | — |
| Tools enabled | `code_execution`, `file`, `terminal`, `web`, `memory`, `skills` |
...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/data-engineer/SOUL.md`) |
...
```bash
# create
hermes profile create data-engineer --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/data-engineer/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/data-engineer/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/data-engineer/.env   # REQUIRED: keys
# use
hermes -p data-engineer chat
```
