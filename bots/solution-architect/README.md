...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/solution-architect/SOUL.md`) |
...
```bash
# create
hermes profile create solution-architect --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/solution-architect/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/solution-architect/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/solution-architect/.env   # REQUIRED: keys
# use
hermes -p solution-architect chat
```
