...
| Tools enabled | `session_search`, `memory`, `skills`, `todo` |
...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/cto/SOUL.md`) |
...
```bash
# create
hermes profile create cto --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/cto/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/cto/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/cto/.env   # REQUIRED: keys
# use
hermes -p cto chat
```
