...
| Fallback chain | — |
| Tools enabled | `terminal`, `computer_use`, `file`, `skills` |
...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/it-support/SOUL.md`) |
...
```bash
# create
hermes profile create it-support --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/it-support/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/it-support/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/it-support/.env   # REQUIRED: keys
# use
hermes -p it-support chat
```
