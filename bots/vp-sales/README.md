...
| Fallback chain | — |
| Tools enabled | `web`, `x_search`, `memory`, `tts` |
...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/vp-sales/SOUL.md`) |
...
```bash
# create
hermes profile create vp-sales --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/vp-sales/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/vp-sales/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/vp-sales/.env   # REQUIRED: keys
# use
hermes -p vp-sales chat
```
