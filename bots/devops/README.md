...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/devops/SOUL.md`) |
...
```bash
# create
hermes profile create devops --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/devops/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/devops/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/devops/.env   # REQUIRED: keys
# use
hermes -p devops chat
```
