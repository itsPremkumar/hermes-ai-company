...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/qa-engineer/SOUL.md`) |
...
```bash
# create
hermes profile create qa-engineer --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/qa-engineer/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/qa-engineer/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/qa-engineer/.env   # REQUIRED: keys
# use
hermes -p qa-engineer chat
```
