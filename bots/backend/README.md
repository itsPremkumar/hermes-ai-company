...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/backend/SOUL.md`) |
...
```bash
# create
hermes profile create backend --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/backend/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/backend/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/backend/.env   # REQUIRED: keys
# use
