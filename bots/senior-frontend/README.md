...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/senior-frontend/SOUL.md`) |
...
```bash
# create
hermes profile create senior-frontend --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/senior-frontend/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/senior-frontend/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/senior-frontend/.env   # REQUIRED: keys
# use
hermes -p senior-frontend chat
```
