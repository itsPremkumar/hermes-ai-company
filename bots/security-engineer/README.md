...
| Fallback chain | — |
| Tools enabled | `terminal`, `file`, `code_execution`, `session_search`, `skills` |
...
| File | Purpose |
|---|---|
| `SOUL.md` | persona + standing orders (deploy to `profiles/security-engineer/SOUL.md`) |
...
```bash
# create
hermes profile create security-engineer --no-skills --description "<see docs/02>"
# deploy identity + config from this folder
cp SOUL.md %HERMES_HOME%/profiles/security-engineer/SOUL.md
cp config.yaml %HERMES_HOME%/profiles/security-engineer/config.yaml
cp %HERMES_HOME%/.env %HERMES_HOME%/profiles/security-engineer/.env   # REQUIRED: keys
# use
hermes -p security-engineer chat
```
